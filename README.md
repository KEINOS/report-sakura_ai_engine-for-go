...
環境変数 `SAKURA_AI_TOKEN` を設定したうえで、検証前に以下のリクエストを行い、API が稼働していることを確認した。
...
`research.py` は環境変数 `SAKURA_AI_TOKEN` を優先して API アクセストークンを解決する。未設定の場合は [Hermes Agent](https://hermes-agent.nousresearch.com/) の credential pool を参照するが、その実行ファイルは `HERMES_PYTHON` 定数のパスに固定されている。再実行時は `SAKURA_AI_TOKEN` の設定を推奨する。トークン値は成果物へ保存していない。
...
Python 環境（`research.py`）には `httpx` が必要である。`SAKURA_AI_TOKEN` を設定せず Hermes Agent の credential pool を使う場合は、環境に合わせて `research.py` の `HERMES_PYTHON` を確認する。

## Sakura AI Engine As Sub-Agent Skill

本調査の結果をモデル選択の根拠として、Sakura AI Engine を汎用的な packet-only sub-agent として利用する [`skills/sakura-ai/`](skills/sakura-ai/) を用意した。特定の AI Agent、SDK、credential pool には依存せず、Python 標準ライブラリと環境変数 `SAKURA_AI_TOKEN` のみを使用する。
...
export SAKURA_AI_TOKEN="your:sakura-ai-engine/api+token+here"
```
...
- [skills/sakura-ai/scripts/sakura_ai.py](skills/sakura-ai/scripts/sakura_ai.py)：`SAKURA_AI_TOKEN` を使って OpenAI API 互換エンドポイントを呼び出す標準ライブラリ製クライアント。
