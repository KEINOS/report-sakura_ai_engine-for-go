...
また、本調査の結果をもとに、Sakura AI Engine を汎用 sub-agent として利用するための [`sakura-ai` skill](#sakura-ai-engine-as-sub-agent-skill) も用意した。

- 調査日：2026 年 6 月 21 日、23 日
- 調査員：Codex (GPT-5.5、推論: Medium)
- 監修・編集：[KEINOS](https://github.com/KEINOS/)
  - 補助 Reviewer：Copilot (`gpt-5.3-codex`)、Hermes (`preview/Qwen3.6-35B-A3B`)、Claude (`opus`)
...
- [制約と次の検証案](#制約と次の検証案)
- [再実行方法](#再実行方法)
- [Sakura AI Engine As Sub-Agent Skill](#sakura-ai-engine-as-sub-agent-skill)
- [検証ファイル一覧](#検証ファイル一覧)
...
> 検証は、[基盤モデル無償プラン](https://ai.sakura.ad.jp/sakura-ai/ai-engine/)で行った。5 回の本試行は各 30 件、計 150 件であり、事前確認などを含む調査期間中の chat completions リクエスト総数は 175 件だった。
...
次の値を評価ベクトルに対応させ、[Pareto front](https://en.wikipedia.org/wiki/Pareto_front) を求めた。ここでは「その候補より全評価軸で同等以上、かつ少なくとも 1 軸で優れる別の候補が存在しない候補の集合」を指す。その中から、選定方針に応じて最終候補を絞り込んだ。

| 評価軸 | Pareto front への入力値 |
| :----- | :----------- |
| Functionality | 3 課題、合計 19 件の通常テストの合格率 |
| Effectiveness | 通常テスト全体に合格した課題数 / 3 |
| Reliability | ParallelMapOrdered の通常テスト合格率と、3 課題の race test 成功率の平均 |
| Usability | 3 課題における `gofmt` 変更不要率、`go vet` 成功率、`golangci-lint` 成功率の平均 |
| Responsiveness | 3 課題の API リクエスト開始から完了までの経過秒数の中央値 |
| Modern Syntax | コンパイル可能かつ `go fix ./...` で変更されなかった課題数 / 3 |
| Cost | 10,000 トークンあたりの入力・出力単価の合計 |

生成行数、benchmark の実行時間・allocation、API 応答のトークン数、ParallelMapOrdered の 20 回反復試験は補助観測として保存したが、Pareto front の算出には使用していない。
...
通常テストまたは race test に失敗したコードは benchmark の対象外とした。生成コードを人手では修正していないが、検証用の一時コピーには通常テスト・vet・lint の後で `go fix ./...` を適用しており、race test と benchmark はその変更後のコードに対して実行した。20 回反復試験は未変更の生成コードを使用し、評価ベクトルには含めていない。
...
環境変数 `SAKURA_AI_API_KEY` を設定したうえで、検証前に以下のリクエストを行い、API が稼働していることを確認した。

```bash
curl --location 'https://api.ai.sakura.ad.jp/v1/chat/completions' \
  --header 'Accept: application/json' \
  --header "Authorization: Bearer ${SAKURA_AI_API_KEY}" \
...
`research.py` は環境変数 `SAKURA_AI_API_KEY` を優先して API アクセストークンを解決する。未設定の場合は [Hermes Agent](https://hermes-agent.nousresearch.com/) の credential pool を参照するが、その実行ファイルは `HERMES_PYTHON` 定数のパスに固定されている。再実行時は `SAKURA_AI_API_KEY` の設定を推奨する。API キーの値は成果物へ保存していない。

```bash
PYTHON=/path/to/python-with-httpx
SAKURA_AI_API_KEY="your:sakura-ai-engine/api+key+here"
...
Python 環境（`research.py`）には `httpx` が必要である。`SAKURA_AI_API_KEY` を設定せず Hermes Agent の credential pool を使う場合は、環境に合わせて `research.py` の `HERMES_PYTHON` を確認する。

## Sakura AI Engine As Sub-Agent Skill

本調査の結果をモデル選択の根拠として、Sakura AI Engine を汎用的な packet-only sub-agent として利用する [`skills/sakura-ai/`](skills/sakura-ai/) を用意した。特定の AI Agent、SDK、credential pool には依存せず、Python 標準ライブラリと環境変数 `SAKURA_AI_API_KEY` のみを使用する。

利用する AI Agent の skills ディレクトリへ、`skills/sakura-ai/` をディレクトリごとコピーする。コピー先で `SKILL.md` が検出されれば、Codex 以外の Agent でも同じ手順とモデル選択基準を利用できる。

```bash
cp -R skills/sakura-ai /path/to/agent/skills/
export SAKURA_AI_API_KEY="your:sakura-ai-engine/api+key+here"
```

推論を行わない API の稼働・認証確認は次のとおり。

```bash
python3 /path/to/agent/skills/sakura-ai/scripts/sakura_ai.py --check
```

レビュー対象や必要な前提を1つのテキストへまとめ、sub-agent にツールやリポジトリへのアクセス権を与えずに送信する。

```bash
python3 /path/to/agent/skills/sakura-ai/scripts/sakura_ai.py \
  --profile reliable \
  --prompt-file review-packet.txt
```

既定の `reliable` は5回とも 18/19 だった `preview/Qwen3.6-35B-A3B` を使用する。速度とのバランスには `balanced`、機械的に棄却可能な予備判定には `rapid`、安価な別観点には `opportunistic`、高遅延・高価格を許容する最高到達点重視には `peak` を選べる。各プロファイルの根拠と制約は [`model-routing.md`](skills/sakura-ai/references/model-routing.md) に記載した。

sub-agent の出力は助言として扱い、ファイル確認、コマンド実行、計測を行ったという自己申告は証拠とみなさない。重要な指摘は、ホスト側の Agent がソース、テスト、または決定的なツールで再確認する。
...
- [skills/sakura-ai/SKILL.md](skills/sakura-ai/SKILL.md)：Sakura AI Engine を汎用 packet-only sub-agent として利用する手順。
- [skills/sakura-ai/scripts/sakura_ai.py](skills/sakura-ai/scripts/sakura_ai.py)：`SAKURA_AI_API_KEY` を使って OpenAI API 互換エンドポイントを呼び出す標準ライブラリ製クライアント。
- [skills/sakura-ai/references/model-routing.md](skills/sakura-ai/references/model-routing.md)：5回の検証結果に基づくモデルプロファイルの選択根拠。
